    ...
    def _connectBridge(self) -> bool:
        if self._bridgeConnectTries < 3:
            try:
                ...
                self._bridge = Bridge(
					ip=managers.ConfigManager.getModuleConfigByName(self.name, 'phueBridgeIp'),
					config_file_path=hueConfigFile
				)
                ...
                if not self._bridge.registered:
                    ...
                    managers.ThreadManager.doLater(
						...
						args=[managers.TalkManager.randomTalk(self.name, 'pressBridgeButtonConfirmation')]
					)
            ...
            managers.ThreadManager.doLater(interval=3, func=managers.MqttServer.say, args=[managers.TalkManager.randomTalk(self.name, 'pressBridgeButtonTimeout')])
            return False
    ...
    def onMessage(self, intent: str, session: DialogSession) -> bool:
        ...
        if self._bridge is None or not self._bridge.registered:
            managers.MqttServer.endTalk(
				sessionId=sessionId,
				text=managers.TalkManager.randomTalk(self.name, 'lightBridgeFailure'),
				client=siteId
			)
            return True
        ...
        if 'Room' in slots:
            for slot in slots['Room']:
                ...
                if room not in self._groups.keys() and room != 'everywhere':
                    managers.MqttServer.endTalk(
						sessionId=sessionId,
						text=managers.TalkManager.randomTalk(self.name, 'roomUnknown').format(room),
						client=siteId
					)
                    return True
        ...
        if 'Scene' in slots:
            for slot in slots['Scene']:
                ...
                if scene not in self._scenes.keys():
                    managers.MqttServer.endTalk(
						sessionId=sessionId,
						text=managers.TalkManager.randomTalk(self.name, 'sceneUnknown').format(scene),
						client=siteId
					)
                    return True
        ...
        if intent == self._INTENT_LIGHT_ON:
            ...
            if 'Room' in slots:
                ...
                managers.MqttServer.endTalk(sessionId, text=managers.TalkManager.randomTalk(self.name, 'roomUnknown').format(siteId), client=siteId)
                return True
            ...
            if 'Scene' not in slots and 'Scene' not in customData:
                ...
                if 'Room' in slots:
                    ...
                    managers.MqttServer.endTalk(sessionId, text=managers.TalkManager.randomTalk(self.name, 'roomUnknown').format(place), client=siteId)
                    return True
        ...
        managers.MqttServer.endTalk(sessionId, text=managers.TalkManager.randomTalk(self.name, 'confirm'))
        return True
