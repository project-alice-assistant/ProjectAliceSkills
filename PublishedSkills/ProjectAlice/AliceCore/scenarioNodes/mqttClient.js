...
module.exports = function (RED) {
	...
	function AliceNode(n) {
		...
		if (this.brokerurl === '') {
			...
			if (this.broker.indexOf('://') > -1) {
				...
				// Only for ws or wss, check if proxy env let for additional configuration
	...
	RED.nodes.registerType('alice', AliceNode, {
		credentials: {
			user: {type: 'text'},
	...
	RED.nodes.registerType('on event', MQTTInNode);
};
